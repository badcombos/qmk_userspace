...
static bool caps_word_bool = false;
...
bool process_case_modes(uint16_t keycode, const keyrecord_t *record) {
	if (caps_word_bool || xcase_state) {
		...
		if (record->event.pressed) {
			...
			else if (caps_word_bool && keycode >= KC_A && keycode <= KC_Z){
